public class AVL {
	...
	{
		Node n = new Node(key); // yeni node oluþturduk
		insert(root,n); // recursive fonksiyona gidiyoruz.
	}
	
	private void insert(Node r, Node n)
	{
		if(r == null) // kök boþ ise elemaný kök yapýyoruz.
		{
			root = n;
		}
		else{
			// kök boþ deðilse, eðer kökten küçükse sol'a, deðilse sað çocuga ekleyeceðiz
			if(r.data > n.data) // sol
			{
				if(r.left == null) // boþ olana kadar recursive olarak geziyoruz.
				{  // çocuklar arasý baðlantý yapýldý
					n.parent = r;
					r.left = n;
				}
